...
// In-place and repeat-safe: never rebuild or reseed an existing table.
if (!db.prepare('PRAGMA table_info(projects)').all().some(column => column.name === 'archivedAt')) {
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/api/projects' && request.method === 'GET') {
      ...
      return send(200, db.prepare(`SELECT * FROM projects WHERE archivedAt IS ${filter === 'true' ? 'NOT ' : ''}NULL ORDER BY id COLLATE BINARY`).all());
    }
    ...
    if (request.method === 'POST' && match[2]) {
      if (match[2] === 'archive') db.prepare('UPDATE projects SET archivedAt = COALESCE(archivedAt, ?) WHERE id = ?').run(new Date().toISOString(), id);
      else db.prepare('UPDATE projects SET archivedAt = NULL WHERE id = ?').run(id);
      return send(200, detail(id));
    }
    if (request.method === 'PATCH' && !match[2]) {
      if (row.archivedAt !== null) return send(409, {error:'ARCHIVED'});
